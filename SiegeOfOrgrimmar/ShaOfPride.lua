...
do
	...
	function mod:TitanGiftApplied(args)
		local _, _, _, prideExpires = self:UnitDebuff(args.destName, auraOfPride, 146817) -- this is to check if the person has aura of pride then later spawn remaining duration bar
		if self:Me(args.destGUID) then
			isOnMe = true
